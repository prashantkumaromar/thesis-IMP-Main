...
So $\{x_{j}\}$ is a Cauchy Sequence and so has its limit in $K$ and is unique also \\
otherwise 
...
Since A and B is convex,  K is also convex and so its closer, $ \bar{K}$ is also convex hence we can apply above theorem for \bar{K} so that we get a unique vector $v \in \bar{K}$ of minimum norm \\
for any $ n \in K, $ 
line segment 
...
from this hyperplane, its half space contains $C$ So $x \notin$ half space of $A$  so 
$$ x \notin \bigcap_{H \in \mathcal{H}} H$$
Hence Done
...
following.
\\ 
We have $\phi_{t}$, 1-parameter family of diffeomorphism of $\mathbb{R}^{n},$ with corresponding time-dependent vector field $$\boldsymbol{X} = \boldsymbol{X}(x, t)$$ and at time zero, \boldsymbol{X} is \\ $$\boldsymbol{\xi}(\boldsymbol{x})=\boldsymbol{X}(\boldsymbol{x}, \boldsymbol{0})$$ and  restircting \boldsymbol{\xi} on its boundary, we have
